...
int nodeAmount = sizeof(path) / 12; // Количество точек. Определяется как вес всего массива / (2+2+2) байта
...
void setup() {
  ...
  planner.setMaxSpeed( StepsPerRev );         // Максимальная скорость
...
void homing() {
  if (digitalRead(conPin0)) {       // если концевик 0 не нажат
    planner.setSpeed(0, -640);       // ось 0, -10 шаг/сек
